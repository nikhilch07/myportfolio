  ...
  return (
    <a href={href} target="_blank" rel="noreferrer" className="block">
      <div className="[perspective:1200px]">
        <div
          ref={ref}
          onMouseEnter={() => enabled && setShineStyle((s) => ({ ...s, opacity: 1 }))}
          onMouseMove={onMove}
          onMouseLeave={reset}
          ...
          style={{ transform }}
        >
          <img src={src} alt={title} className="absolute inset-0 w-full h-full object-cover opacity-90 pointer-events-none" draggable="false" />
          <div className="absolute inset-0 mix-blend-overlay pointer-events-none" style={shineStyle} />
          <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/20 to-transparent pointer-events-none" />
          <div className="absolute bottom-0 left-0 right-0 p-4 text-white">
            <h3 className="text-lg font-semibold">{title}</h3>
            {tech.length > 0 && <p className="mt-1 text-sm opacity-80">{tech.join(' • ')}</p>}
          </div>
        </div>
      </div>
    </a>
...
export default function Showcase3D() {
  ...
  return (
    <section id="showcase-3d" className="py-20 bg-slate-50 dark:bg-slate-900">
      <div className="max-w-6xl mx-auto px-4">
        <h2 className="text-3xl md:text-4xl font-bold tracking-tight">Interactive 3D Project Gallery</h2>
        <p className="mt-2 text-slate-600 dark:text-slate-400">
          Subtle GPU-accelerated tilt that follows the cursor. Automatically disabled on touch devices and for users who prefer reduced motion.
        </p>
